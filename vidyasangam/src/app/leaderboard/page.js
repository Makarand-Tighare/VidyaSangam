...
export default function Leaderboard() {
    ...
    if (!authToken) {
      ...
      (async () => {
        try {
          ...
          const recalculateResponse = await fetch('https://vidyasangam.duckdns.org/api/mentor_mentee/leaderboard/calculate/', {
            headers: {
  ...
  const fetchLeaderboardData = async () => {
    try {
      ...
      const response = await fetch(`https://vidyasangam.duckdns.org/api/mentor_mentee/leaderboard/?role=${activeTab.slice(0, -1)}&search=${debouncedSearchQuery}`, {
        headers: {
          'Authorization': `Bearer ${authToken}`
        }
      });
  ...
  const recalculateLeaderboardPoints = async () => {
    try {
      ...
      const response = await fetch('https://vidyasangam.duckdns.org/api/mentor_mentee/leaderboard/calculate/', {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${authToken}`
        }
  ...
  const syncLeaderboardPoints = async () => {
    try {
      ...
      const response = await fetch('https://vidyasangam.duckdns.org/api/mentor_mentee/leaderboard/sync/', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${authToken}`,
