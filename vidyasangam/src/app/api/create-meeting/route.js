...
export async function POST(req) {
  try {
    ...
    const authCheckResponse = await axios.get('http://54.166.190.24:8000/api/utility/check-auth', { withCredentials: true });

    if (authCheckResponse.data.is_authorized) {
      // If authorized, create the meeting
      const response = await axios.post('http://54.166.190.24:8000/api/utility/create-meet', {}, { withCredentials: true });
      ...
      const authResponse = await axios.get('http://54.166.190.24:8000/api/utility/authorize', { withCredentials: true });
