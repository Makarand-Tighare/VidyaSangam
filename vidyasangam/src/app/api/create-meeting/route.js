...
export async function POST(req) {
  try {
    ...
    const authCheckResponse = await axios.get('http://localhost:5003/check-auth', { withCredentials: true });

    if (authCheckResponse.data.is_authorized) {
      // If authorized, create the meeting
      const response = await axios.post('http://localhost:5003/create-meet', {}, { withCredentials: true });
      ...
      const authResponse = await axios.get('http://localhost:5003/authorize', { withCredentials: true });
